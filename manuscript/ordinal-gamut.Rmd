...
author: "Claudia Solis-Lemus$^+$, Taylor Fischer$^+$, Cuining Liu, Elizabeth Leslie, Debashis Ghosh, David Cutler, Michael Epstein"
...
is a case with family history of the disease.
...
we defined the person as a control. If the proband was affected, we defined the person 
...
For family history, we simulated three different scenarios: 1) parents as family members, 
2) one sibling as family member, or 3) two siblings as family members.
We considered sample size $N=400,750,1000,1500$ subjects per group: controls, 
cases without family history and cases with family history.
Finally, similar to GAMuT [@Broadaway2016], we use the weighting scheme recommended by @Wu2011: 
$w_v=Beta(MAF_v,1,25)/Beta(0,1,25)$ for rare variant $v$ in the simulations and analyses of this
manuscript.
...
Figure \@ref(fig:qq) shows the quantile-quantile (QQ) plots of 10,000 null simulations with 750 subjects per group (2250 in total), target disease prevalence of $0.01$, and $\lambda=2$. We show the comparison with ordinal GAMuT, SKAT and burden test. All methods compared properly control the type I error.
...
We introduced an extension to the GAMuT method [@Broadaway2016] to incorporate family information to enhance case-control association studies. This approach converts the usual binary phenotype of case-control status into an ordinal phenotype with three levels: cases with family history, cases without family history and controls, and it allows to adjust for covariates. Just as the standard GAMuT test, the ordinal GAMuT obtains analytic p-values from Davies' method [@Davies1980] which is computationally efficient, allowing the analysis of datasets in the genomic scale.
...
The extension of GAMuT to include family history information could lead to future lines of work. In particular, potential options for enhanced outcome variables could involve conditional means from liability-threshold models.
