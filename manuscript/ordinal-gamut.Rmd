--- 
title: "Enhanced Case-Control Association Analyses of Rare Variation Leveraging Family History"
author: "Claudia R. Solis-Lemus$^+$, S. Taylor Fischer$^+$, Cuining Liu, Elizabeth J. Leslie, David J. Cutler, Debashis Ghosh, Michael P. Epstein"
...
\noindent \textbf{Abstract.} Standard methods for case-control association studies of rare variation often treat disease outcome as a dichotomous
...
enriched for risk variation relative to sporadic cases. 
...
While case-control studies are limited by the difficulty of matching cases and controls, cohort studies might not include a sufficiently large sample of cases. This is evident, for example, in diseases with very low prevalence, or in cohorts that do not include the specific demographic group with highest disease prevalence. Thus, achieving a sufficient sample size of cases is challenging for both cases-control studies and cohort studies.
In case-control studies, collecting cases can be expensive and time-consuming, while in cohort studies, it can be complicated by disease prevalence and sampling scheme.
...
In short, they propose a study design in which family members of cases are identified, instead of cases directly (GWAX: genome-wide association study by proxy). In GWAX, proxy cases are defined as the family members of affected individuals and controls are defined as the family members of unaffected individuals.

Here, we extend the idea in @Liu2017 by incorporating information of family members in existing cases and controls. That is, while the disease status in GWAX has two categories, proxy-cases and controls, we extend the disease status to four categories: cases with family history, cases without family history, controls with family history and controls without family history.
Furthermore, we allow for the study of multiple phenotypes simulatenously by extending GAMuT [@Broadaway2016]. We denote our proposed methodology "Ordinal GAMuT".
...
The structure of this paper is as follows: after introducting the ordinal GAMuT method using the kernel-distance covariance framework (KDC)
...
is a case with family history of the disease. Here we describe the case when there are no controls with family history. However, this ordinal approach can easily be extended to the case of four categories: controls without ($\mathbf{\tilde{P}}_j=0$) and with family history ($\mathbf{\tilde{P}}_j=1$), and cases 
without ($\mathbf{\tilde{P}}_j=2$) and with family history ($\mathbf{\tilde{P}}_j=3$).

Similarly to GAMuT, we construct a centered similarity matrix for the ordinal 
...
the family-history information, we account for other covariates by regressing the phenotypes on covariates 
with a 
...
Note that for an ordinal response with $j$ categories, we fit $j-1$ proportional odds models. Thus, in our particular setting, we have three categories: controls ($j=0$), cases without family history ($j=1$) and cases with family history ($j=2$), and thus, we will fit 2 proportional odds models. Each of these models will produce a set of scalar residuals, which will be then combined into a two-dimensional vector residuals, one per model. Thus, the residuals of the proportional odds model are then inputted into the GAMuT framework as the multiple phenotypes. These residuals are not influenced by the confounding covariates anymore, and thus, can be directly tested for association with the genotypes.
...
with effect size for each causal variant given by $C|\log_{10}(MAF)|$, where we varied as $C=4,6$. This setup defines the 
...
as case without family history if one or more family members were unaffected or a case with family 
history if family members were affected. For the type I error and power simulation, we solely present the results corresponding to the simulations involving parents as family members. For the illustration of the enrichment of causal variants in cases with family history, we show simulations involving one or two siblings.
...
We envision two main future extensions of ordinal GAMuT: 1) to include information of more distant relatives (currently using only parents or siblings), and 2) to use disease liability as continuous phenotype instead of a categorical phenotype.
