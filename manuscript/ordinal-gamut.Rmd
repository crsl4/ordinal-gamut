...
Furthermore, we allow for the study of multiple phenotypes simulatenously by extending GAMuT [@Broadaway2016] to the newly defined categorical response. We denote our proposed methodology "Ordinal GAMuT".
...
Note that for an ordinal response with $j$ categories, we fit $j-1$ proportional odds models. Thus, in our particular setting, we have three categories: controls ($j=0$), cases without family history ($j=1$) and cases with family history ($j=2$), and thus, we will fit 2 proportional odds models. 
With these models, we will estimate the multinomial probabilities, which will be combined into a three-dimensional vector to be then inputted into the GAMuT framework as the multiple phenotypes. 
...
as case without family history if all family members were unaffected or a case with family 
history if one or more family members were affected. For the type I error and power simulation, we solely present the results corresponding to the simulations involving parents as family members. For the illustration of the enrichment of causal variants in cases with family history, we show simulations involving one or two siblings.
...
knitr::include_graphics(c('figures/scenario1.png','figures/scenario3-2.png','figures/scenario4.png','figures/scenario5.png'))
```
