...
Case-control association studies are among the most influential methods to identify rare genetic variants that are associated with complex disease. In these studies, individuals with the disease (cases) and individuals without the disease (controls) are genotyped, and the causal effects of each genetic variant are estimated as a function of the difference in allele frequency between cases and controls, after accounting for potential confounders.

These studies use two different types of technologies: 1) next-generation sequencing which allows for the inclusion of all single-nucleotide variants in the human genome, and 2) Illumina chips which  provides genotypes for different markers, primarily protein-altering variants.
Regardless of the data technology, evidence shows that these association studies can have insufficient power due to small effect sizes, low frequency, or small sample sizes.

@Zollner2012 showed than statistical power can be increased by amplifying the frequency difference of risk variants between cases and controls. This could be accomplish by the inclusion of family information. On one hand, cases with family history are enriched for risk variation over sporadic cases because one or more risk variants tend to be segregated in families with multiple affected individuals [@Epstein2015, @Zollner2012]. On the other hand, controls with family history are enriched over controls without family history as well [@Liu2017]. 


Here, we extend GAMuT [@Broadaway2016], a nonparametric association test for high-dimensional phenotypes and high-dimensional genotypes by differentiating between cases with family history, and cases without family history. This distinction yields an ordinal phenotype with three levels: controls ($0$), sporadic cases ($1$), and cases with a family history ($2$).
We denote our proposed methodology "Ordinal GAMuT".
Family history information will enrich for causal variation, and our approach is not restricted to a specific set of categories. In fact, our method can even be extended to the case of four categories: cases with family history, cases without family history, controls with family history and controls without family history.

Aside from allowing for an arbitrary number of genotypes and phenotypes, ordinal GAMuT can also correct for confounding covariates
such as population stratification. Furthermore, just as the standard GAMuT, the newly proposed ordinal GAMuT has analytical p-values,
