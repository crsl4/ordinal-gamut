...
ordinal phenotype by defining the ordinal score as $\tilde{P}_j$ = $0$ 
if the subject is a control, $\tilde{P}_j =1$ if the subject is a case 
without family history of the disease, $\tilde{P}_j =2$ if the subject 
is a case with family history of the disease. Here we will focus on the situation when there are no controls with family history in the sample. However, this ordinal approach can easily be extended to the case of four categories: controls without ($\tilde{P}_j=0$) and with family history ($\tilde{P}_j=1$), and cases 
...
To illustrate this procedure, let $\tilde{P}$ be an ordinal response with $M$ categories, and let $P(\tilde{P} \leq j)$ be 
...
$$
\logit P(\tilde{P} \leq j | \mathbf{x}) = \theta_j - \mathbf{\beta}^T\mathbf{x} 
$$
...
odds of $P(\tilde{P} \leq j|\mathbf{x}_1)$ and $P(\tilde{P} \leq j|\mathbf{x}_2)$ do not 
depend on the specific category $j$.
That is,
$$
\frac{P(\tilde{P} \leq j | \mathbf{x}_1)/(1-P(\tilde{P} \leq j | \mathbf{x}_1))}{P(\tilde{P} \leq j | \mathbf{x}_2)/(1-P(\tilde{P} \leq j | \mathbf{x}_2))} = \exp(-\mathbf{\beta}^T(\mathbf{x}_1-\mathbf{x}_2))
$$
This is also denoted a \textit{parallelism assumption} on $\mathbf{\beta}$ [@Yee2010].

Note that for an ordinal response with $j$ categories, we fit $j-1$ proportional odds models. Thus, in our particular setting, we have three categories: controls ($j=0$), cases without family history ($j=1$) and cases with family history ($j=2$), and thus, we will fit 2 proportional odds models: $\logit P(\tilde{P} \leq 0)$ and $\logit P(\tilde{P} \leq 1)$.
...
$$
\mu_{i,0} = P(\tilde{P}_i=0),
\mu_{i,1} = P(\tilde{P}_i=1),
\mu_{i,2} = P(\tilde{P}_i=2)
$$
